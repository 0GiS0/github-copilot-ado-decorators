...
- **Service connection pattern in ADO tasks:** `connectedService:ExternalServer` input type allows tasks to reference Generic service connections. The task SDK provides `tl.getEndpointAuthorizationParameter(endpointName, 'password', false)` to read the credential at runtime. Must call `tl.setSecret()` on the retrieved value.
- **Key architectural insight:** Service connections are the ADO-native pattern for external credentials. "Grant access to all pipelines" solves the decorator scaling problem — no per-pipeline variable group linking needed.
- **Documentation created (2026-03-03):** Issue #13 — Created `README.md` and updated `overview.md`. README covers: description, how-it-works flow, prerequisites, installation (extension + service connection), configuration (opt-out variable, task inputs), output locations (Pipeline Run Summary + step output), architecture module table, troubleshooting table, contributing, and MIT license. Overview.md rewritten for marketplace scanning — concise, feature-focused, with getting started steps. Noted logo.png is pending. Bumped dev version to 0.4.1. PR #30 on branch `feature/13-readme`.
- **Key doc insight:** overview.md is the marketplace listing page — needs to be scannable with clear feature bullets and quick-start steps. README.md is the repo landing page — can be more comprehensive with troubleshooting tables and architecture details. Different audiences, different depth.
